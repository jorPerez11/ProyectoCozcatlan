...
export const AuthProviderAdmin = ({ children }) => {
    ...
    const login = async (email, password, rememberMe = false) => {
        try {
            const response = await fetch(`${API_URL}/loginAdmin`, { // Ajusta si es diferente
                method: "POST",
            ...
            if (userFromApi) {
                ...
                setUser(decodedToken ? { id: decodedToken.id, userType: decodedToken.userType || "admin" } : null);
            }

            toast.success("Inicio de sesión exitoso");
            navigate("/dashboardPrivate"); // Ruta de Admin
            return true;
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const checkAuth = async () => {
            ...
            try {
                ...
                if (decoded) {
                    setUser({ id: decoded.id, userType: decoded.userType || "admin" });
                }
            } catch (error) {
                clearSession(false);
            } finally {
    ...
    return (
        <AuthContextAdmin.Provider value={{ user, setUser, authCookie, logout, login, loading, API: API_URL }}>
            {children}
